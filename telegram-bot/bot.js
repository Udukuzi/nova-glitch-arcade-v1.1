// Nova Glitch Arcade Telegram Bot
...
bot.command('play', async (ctx) => {
  ...
  gameButtons.push([Markup.button.webApp('🌐 Open Full App', WEBAPP_URL)]);
...
bot.action('swap', async (ctx) => {
  ...
  await ctx.reply(
    '*💱 Token Swap*\n\nSwap between NAG, USDC, USDT, and SOL',
    {
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [Markup.button.webApp('🔄 Open Swap', `${WEBAPP_URL}/swap`)],
  ...
  try {
    ...
    await ctx.reply(
      `*🏆 Top Players:*\n\n${leaders}\n\n_Your rank: #127_`,
      {
        parse_mode: 'Markdown',
        ...Markup.inlineKeyboard([
          [Markup.button.webApp('📊 Full Leaderboard', `${WEBAPP_URL}/leaderboard`)]
        ])
      }
    );
  } catch (error) {
