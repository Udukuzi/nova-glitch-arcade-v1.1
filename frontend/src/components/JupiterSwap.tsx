/**
 * Jupiter Swap Widget for Nova Arcade Glitch
...
export default function JupiterSwap({ open, onClose, onSwapComplete }: JupiterSwapProps) {
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      ...
      try {
        // Check if both tokens exist on Jupiter
        const [inputExists, outputExists] = await Promise.all([
          checkTokenExists(inputToken.mint),
          checkTokenExists(outputToken.mint)
        ]);

        if (!inputExists) {
          throw new Error(`${inputToken.symbol} token not found on Jupiter`);
        }
        if (!outputExists) {
          throw new Error(`${outputToken.symbol} token not found on Jupiter`);
        }
        ...
        if (errorMessage.includes('No routes found') || errorMessage.includes('Token not found')) {
          setError(`${outputToken.symbol} token may not be available for swapping yet. Try again later or use a different token.`);
        } else if (errorMessage.includes('Insufficient liquidity')) {
          setError('Insufficient liquidity for this swap amount. Try a smaller amount.');
        } else {
        ...
        <>
          ...
          <motion.div
            ...
            style={{
              ...
              background: 'linear-gradient(135deg, #0e0f15 0%, #1a0f3d 50%, #0e0f15 100%)',
              border: '2px solid #6b46c1',
              borderRadius: 24,
              padding: 32,
              width: '100%',
              maxWidth: 480,
              boxShadow: '0 0 60px rgba(107, 70, 193, 0.4), 0 20px 40px rgba(0, 0, 0, 0.3)',
              zIndex: 1000
            }}
          >
              {/* Header */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <h2 style={{ 
                  ...
                  fontSize: 28, 
                  fontWeight: 'bold',
                  background: 'linear-gradient(135deg, #8af0ff, #6b46c1)',
              ...
              <div style={{ textAlign: 'center', marginTop: 16 }}>
                <span style={{ color: '#8af0ff', fontSize: 12, opacity: 0.7 }}>Powered by Jupiter Aggregator V6</span>
              </div>
          </motion.div>
        </>
      )}
