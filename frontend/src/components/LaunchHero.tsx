...
const DEV_BYPASS = false; // Production mode - enforce token requirements
...
export default function LaunchHero({ session, onSession, connectSignal, onEnter }: LaunchHeroProps) {
  ...
  // Play intro sound on first user interaction
  const playIntroSound = () => {
    if (!introSoundPlayed && userInteracted) {
      try {
  ...
  useEffect(() => {
    const handleFirstInteraction = () => {
      if (!userInteracted) {
        ...
        console.log('🎵 First interaction detected - playing intro sound');
        // Play intro sound on first click/touch with small delay
        setTimeout(() => {
          if (!introSoundPlayed) {
            try {
              soundManager.playGameStart();
