 ...
 * Handles all Jupiter swap operations for Nova Arcade Glitch
 */
 ...
 */
export async function checkTokenExists(mint: string): Promise<boolean> {
  try {
    const response = await fetch(`${JUPITER_PRICE_API}/price?ids=${mint}`);
    const data = await response.json();
    return data.data && data.data[mint] !== undefined;
  } catch {
    return false;
  }
}
...
export async function getQuote(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to get quote: ${response.statusText} - ${JSON.stringify(errorData)}`);
    }
    
    const result = await response.json();
    console.log('✅ Jupiter Quote Success:', result);
    return result;
  } catch (error) {
