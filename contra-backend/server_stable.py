...
@app.route("/")
def index():
    return "✅ Contra backend fully headless and verified."
...
def game_loop():
    ...
    while True:
        try:
            screen.fill((5, 5, 15))
            pygame.draw.rect(screen, (255, 50, 50), (x % 560, 220, 80, 80))
...
@socketio.on("connect")
def on_connect():
    print("🔗 Frontend connected via SocketIO")
...
if __name__ == "__main__":
    start_game()
    print("🔥 Contra backend running at http://127.0.0.1:5001 (headless, stable mode)")
